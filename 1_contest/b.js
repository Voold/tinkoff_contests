...
function binary_search (arr, element){
    ...
    if (Math.abs(element - arr[left_pos]) <= Math.abs(element - arr[left_pos+1])){
        return `${arr[left_pos]}`;
    } else {
        return `${arr[left_pos+1]}`;
    }
...
function calculating (lines_arr) {
    ...
    for (let i = 0; i < k; i++){
        ...
        if (
            ...
            questions[i] = `${questions[i] == sorted_arr[0]? sorted_arr[0] : sorted_arr.at(-1)}`;
            continue;
        }
